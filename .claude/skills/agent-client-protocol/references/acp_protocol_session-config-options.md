...
source_url: https://agentclientprotocol.com/protocol/session-config-options
---
...
> **Info:** Session Config Options are the preferred way to expose session-level configuration. If an Agent provides `configOptions`, Clients **SHOULD** use them instead of the [`modes`](./session-modes) field. Modes will be removed in a future version of the protocol.
...
**type** `ConfigOptionType` *required*
...
**options** `ConfigOptionValue[]` *required*
...
> **Warning:** Categories are for UX purposes only and **MUST NOT** be required for correctness. Clients **MUST** handle missing or unknown categories gracefully.
