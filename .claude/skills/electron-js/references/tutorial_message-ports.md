...
source_url: https://www.electronjs.org/docs/latest/tutorial/message-ports
---
...
`MessagePort` objects can be created in either the renderer or the main process, and passed back and forth using the `ipcRenderer.postMessage` and `WebContents.postMessage` methods. Note that usual IPC methods like `send` and `invoke` cannot be used to transfer `MessagePort`s, only the `postMessage` methods can transfer `MessagePort`s.
...
app.whenReady().then(async () => {
  ...
  // webContents.postMessage is available after the page has loaded
...
Then, in your preload scripts you receive the port through IPC and set it up.

```javascript
// Preload (preloadMain.js and preloadSecondary.js)
...
In the renderer, you can then use the port to send and receive messages:

```javascript
// Renderer
// sendMessage will be called from the UI
window.electronMessagePort.postMessage('ping')
```

### Worker process

In this example, your app has a renderer process that acts as a hidden window for computation. You want the app window to be able to communicate directly with the worker process, without the performance overhead of relaying via the main process.
...
ipcMain.on('give-me-a-stream', (event, msg) => {
  ...
  // didn't explicitly close the port, it would eventually be garbage-collected,
  // which would also trigger the 'close' event in the renderer.
  replyPort.close()
})
```
...
When context isolation is enabled, IPC messages from the main process are delivered to the isolated world, rather than the main world. Sometimes you want to deliver messages to the main world directly, without having to step through the isolated world.
